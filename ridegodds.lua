...
-- RideGo Farm — FINAL v25.1
-- Giu nguyen logic v25. Chinh: STEP_DIST=200, ascend bang CFrame 1 phat.
...
local function flyUnderground(target)
    ...
    -- ===== CFrame THANG LEN MAT DAT TAI KHACH =====
    car = myCar or findMyCar()
    if car and reached then
        setState("under - cframe len mat dat")

        -- Lay mat dat thuc te tai khach (co the tren platform/cau cao)
        local realFloor = floorBelow(target) or targetFloor
        local upTargetY = math.max(realFloor + CRUISE_Y, target.Y + 30)

        local curPivot = car:GetPivot()
        local rot = curPivot - curPivot.Position
        local dest = Vector3.new(target.X, upTargetY, target.Z)
        pcall(function() car:PivotTo(CFrame.new(dest) * rot) end)
        task.wait(0.15)

        -- Dat chinh xac tren mat dat bang bbox
